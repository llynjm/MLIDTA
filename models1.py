"""
多模态 DTA 预测模型 —— 双通路 BiCrossAttention + CoAttention 融合版

核心设计原则
============
图模态失效的根本原因不是 padding mask，而是：
  graph_ban(drug_graph × pro_graph) 与 seq_ban(drug_seq × pro_seq)
  捕捉的是【同质信息】（都是"药物特征 × 蛋白特征"的双线性交互），
  梯度严重冗余，图路径没有机会贡献任何增量信息。

本版本的核心修改
================
保留序列 BAN，同时增加图通路和序列通路的双向交叉注意力，
再使用 CKD-style CoAttentionLayer 对图交互向量和序列交互向量二级融合：

  Stream-1  seq_ban(drug_seq, pro_seq)              → [B, 256]
            序列层面的结合模式匹配（原有，保留）

  Stream-2  BiCrossAttention(drug_graph_nodes ↔ pro_graph_nodes)
            → drug_graph_inter/pro_graph_inter → graph_inter_out [B, 128]
            图通路结构交互，使用 drug_node_mask/pro_node_mask 屏蔽 padding

  Stream-3  BiCrossAttention(drug_seq ↔ pro_seq)
            → drug_seq_inter/pro_seq_inter → seq_inter_out [B, 128]
            序列通路片段交互

  Stream-4  CoAttentionLayer(pro_graph_inter, drug_graph_inter,
                             pro_seq_inter, drug_seq_inter) → [B,128]
            图交互向量与序列交互向量二级融合

  Stream-5  GatedFusion(drug_seq_global, pro_seq_global)  → [B, 256]
            序列全局互信息（轻量）

  Stream-6  GatedFusion(drug_graph_global, pro_graph_global) → [B, 256]
            图全局互信息（轻量）

MLP 输入   = 256 + 128 + 128 + 128 + 256 + 256 = 1152
"""
...
# ─────────────────────────────────────────────────────────────────────────────
N_HEADS  = 4
LSTM_OUT = 256    # BiLSTM 输出维度 (128 × 2方向)
...
# ─────────────────────────────────────────────────────────────────────────────
# BANLayer（仅用于序列-序列交互，序列已经固定长度，不需要 mask）
# ─────────────────────────────────────────────────────────────────────────────
class BANLayer(nn.Module):
    """双线性注意力网络（Bilinear Attention Network）"""
...
class BiCrossAttention(nn.Module):
    ...
    输入两组局部特征 A=[B, N_a, D] 和 B=[B, N_b, D]，分别计算
    A<-B 与 B<-A 两个方向的 cross-attention。每个方向的节点级输出
    使用“注意力池化 + 最大池化”压缩为 [B, out_dim]，避免关键节点
    被普通节点平均稀释。
    """
    ...
    def __init__(self, dim: int = 128, out_dim: int = 128,
        ...
        super().__init__()
        assert out_dim % n_heads == 0, "out_dim 须被 n_heads 整除"
...
#     """
#     CKDmodels.CoAttentionLayer 的同构实现。

#     输入四个 [B, dim] 向量：
#       v0, q0: 图通路双向交互池化后的蛋白侧/药物侧向量
#       v1, q1: 序列通路双向交互池化后的蛋白侧/药物侧向量
#     输出 [B, 4*dim]。
#     """

#     def __init__(self, dim=128, k=196):
#         super().__init__()
...
#         m0 = v0 * q0
#         h_m = self.W_m(m0)
#         h_v = torch.tanh(self.W_v(v1) * h_m)
#         h_q = torch.tanh(self.W_q(q1) * h_m)
#         alpha_v = F.softmax(self.W_h_v(h_v), dim=-1)
#         alpha_q = F.softmax(self.W_h_q(h_q), dim=-1)
#         vector1 = alpha_v * v1
#         vector2 = alpha_q * q1
#         v_0 = vector1 * vector2
#         return torch.cat([vector1, vector2, m0], dim=1)
class CoAttentionLayer(nn.Module):
    """
    多头交叉注意力版协同融合层 —— 替代原 CKD‑style 门控实现

    输入：
      v0, q0 : [B, dim]  图通路双向交互池化后的蛋白侧/药物侧向量
      v1, q1 : [B, dim]  序列通路编码器的全局特征（蛋白侧/药物侧）

    工作原理：
      1. 将每个全局向量按头数拆分为“多个子空间 token”，
         得到 [B, num_heads, head_dim] 的序列表示。
      2. 以 v0 为 Query，v1 为 Key/Value，执行交叉注意力 → out_v
         以 q0 为 Query，q1 为 Key/Value，执行交叉注意力 → out_q
      3. 保留显式的逐元素交互项 m0 = v0 * q0 （提供线性互补信息）
      4. 拼接 [out_v, out_q, m0] → 输出 [B, 3*dim]

    优点：
      - 多头机制可捕获多个子空间中的跨通路交互
      - 图特征主动“查询”序列特征，比被动门控更灵活
      - 输出维度与原版一致，直接兼容后续 FusionRefine(384→256)

    参数：
      dim       : 输入特征维度（默认 128）
      num_heads : 注意力头数（默认 4）
      dropout   : 注意力 dropout（默认 0.1）
      k         : 保留参数占位，无实际作用（兼容旧接口）
    """
    ...
    def __init__(self, dim=128, num_heads=4, dropout=0.1, k=196):
        ...
        # 可选的 LayerNorm（用于残差，但本实现不使用残差，因为全局向量长度=1）
        # 保留以保持模块结构整洁

    def _split_heads(self, x):
        """将 [B, dim] 拆分为 [B, num_heads, head_dim]"""
        B = x.size(0)
        return x.view(B, self.num_heads, self.head_dim)

    def _attention(self, query, key, value):
        """缩放点积注意力，query/key/value 形状均为 [B, h, d]"""
    ...
    def forward(self, v0, q0, v1, q1):
        ...
        # 2. 双向多头交叉注意力
        attn_v = self._attention(Q_v, K_v, V_v)       # v0 查询 v1
        attn_q = self._attention(Q_q, K_q, V_q)       # q0 查询 q1
...
class LinkAttention(nn.Module):
    ...
    def forward(self, x, masks):
        """
        x:     [B, L, input_dim]
        masks: [B, n_heads, L]    1=有效 0=padding
        返回: ([B, input_dim], [B, n_heads, L])
        """
        ...
        # ★ sum(dim=1) 代替 squeeze()，batch_size=1 时不出错
        out = torch.matmul(a, x).sum(dim=1)              # [B, input_dim]
        return out, a


def generate_masks(adj, adj_sizes, n_heads):
    """生成 padding mask，adj_sizes 为每条序列的真实长度（numpy int 数组）"""
...
class LSTMEncode(nn.Module):
    ...
    返回:
      lengths    : numpy int[B]    真实序列长度
      seq_feat   : [B, L_out, 128] 固定长度序列特征（AdaptiveAvgPool）
      masks      : [B, n_heads, L] padding mask（原始长度空间）
      global_feat: [B, 128]        全局池化特征
      attn       : [B, n_heads, L] 注意力权重
    """
    ...
    def forward(self, x_embed, x_idx=None):
        """
        x_embed: [B, L, 128]  Embedding 后的序列
        x_idx  : [B, L] int   原始 token index（用于计算真实长度）
        """
        if x_idx is not None:
            lengths = (x_idx != self.padding_idx).sum(dim=1).cpu().numpy().astype(int)
        else:
            # fallback：从嵌入向量推断（全零 = padding）
...
class EmbeddedSequenceCNN(nn.Module):
    """
    多层 1D-CNN 序列编码器，输入为已经 embedding 好的序列。

    该分支专供 Seq BiCrossAttention 使用，用于构造与 BAN 输入解耦的
    局部 motif / 多尺度片段特征。
    """

    def __init__(self, out_seq_len, embedding_dim=128,
                 num_filters=(128, 128, 128), kernel_size=(3, 5, 7),
                 dropout=0.2):
        super().__init__()
        ...
        self.dropout = nn.Dropout(dropout)
        self.adapt_pool = nn.AdaptiveAvgPool1d(out_seq_len)
        self.out_proj = nn.Linear(num_filters[-1], 128)

    def forward(self, x_embed, token_idx=None, padding_idx=0):
        """
        x_embed:   [B, L, 128] 已 embedding 的序列
        token_idx: [B, L]      原始 token id，用于屏蔽 padding
        """
        if token_idx is not None:
            mask = (token_idx != padding_idx).float().unsqueeze(-1)
        ...
        x = self.adapt_pool(x).transpose(1, 2)
...
class DrugGraphEncoder(nn.Module):
    ...
    def forward(self, x, edge_index, batch, return_node_index=False):
        """
        返回:
          node_feat:   [B, max_nodes, out_dim]   padding 位置已清零
          global_feat: [B, out_dim]
          node_mask:   [B, max_nodes] bool，True = 有效节点
        """
...
class ProteinGraphEncoder(nn.Module):
    ...
    def forward(self, x, edge_index, batch, return_node_index=False):
        """
        返回:
          node_feat:   [B, fixed_nodes, out_dim]   padding 位置已清零
          global_feat: [B, out_dim]
          node_mask:   [B, fixed_nodes] bool，True = 有效节点
        """
...
class GINConvNet(nn.Module):
    """
    多模态 DTA 预测模型（跨模态图-序列交叉注意力版）

    五路融合流（信息互补，无冗余）
    ────────────────────────────────
    Stream-1  seq_ban(drug_seq[B,50,128], pro_seq[B,256,128])
              → [B, 256]
              序列-序列双线性注意力，捕捉主要结合模式

    Stream-2  BiCrossAttention(drug_graph_nodes[B,64,128] ↔ pro_graph_nodes[B,N,128])
              → graph_inter_out [B, 128]
              药物图与蛋白图结构交互，使用两侧图节点 mask

    Stream-3  BiCrossAttention(drug_seq[B,50,128] ↔ pro_seq[B,256,128])
              → seq_inter_out [B, 128]
              药物序列与蛋白序列交互，使用池化后的序列 mask

    Stream-4  CoAttentionLayer(pro_graph_inter, drug_graph_inter,
                               pro_seq_inter, drug_seq_inter)
              → co_out [B,128]
              CKD-style 二级协同注意力融合

    Stream-5  GatedFusion(drug_seq_global[B,128], pro_seq_global[B,128])
              → [B, 128]
              序列全局互信息

    Stream-6  GatedFusion(drug_graph_global[B,128], pro_graph_global[B,128])
              → [B, 128]
              图全局互信息
    ...
    def __init__(self,
        ...
        self.drug_seq_cnn = EmbeddedSequenceCNN(
            out_seq_len=drug_seq_len,
            embedding_dim=128,
            num_filters=(128, 128, 128),
            kernel_size=(3, 5, 7),
            dropout=dropout
        )
        self.pro_seq_cnn = EmbeddedSequenceCNN(
            out_seq_len=pro_seq_len,
            embedding_dim=128,
            num_filters=(128, 128, 128),
            kernel_size=(3, 5, 7),
            dropout=dropout
        ...
        # ── Stream-1: 序列-序列 BAN ───────────────────────────────────────────
        # 序列经 AdaptPool 已是固定长度，无 padding，无需 mask
        ...
        # ── MLP 预测头 ────────────────────────────────────────────────────────
        # 输入: 256 + 128 + 128 + 128 + 256 + 256 = 1152
    ...
    def forward(self, data_mol, d_frags, data_pro, return_attention=False):
        """
        data_mol.x:           [total_drug_nodes, drug_node_dim]
        data_mol.edge_index:  [2, total_drug_edges]
        data_mol.batch:       [total_drug_nodes]
        data_mol.drug_smiles: [B, drug_max_len]    token index（含 padding 0）

        data_pro.x:           [total_pro_nodes, pro_node_dim]
        data_pro.edge_index:  [2, total_pro_edges]
        data_pro.batch:       [total_pro_nodes]
        data_pro.target:      [B, pro_max_len]     token index（含 padding 0）
        """
        ...
        # drug_node_feat:   [B, drug_max_nodes=64, 128]   (padding 位置已清零)
        # drug_graph_global:[B, 128]
        # drug_node_mask:   [B, 64]  bool
        ...
        # pro_node_feat:   [B, pro_fixed_nodes=256, 128]
        # pro_graph_global:[B, 128]
        # pro_node_mask:   [B, 256] bool
        ...
        # drug_seq_feat: [B, 50,  128]  ← 固定长度，无 padding
        # drug_s1:       [B, 128]       ← 序列全局特征
        ...
        # pro_seq_feat: [B, 256, 128]  ← 固定长度，无 padding
        # pro_s1:       [B, 128]
        ...
        # ── Stream-2: 药物图全局特征 ↔ 蛋白图全局特征 双向交叉注意力 ──────────
        # 将图编码器得到的全局表示视为长度为 1 的 token 序列：
        # drug_graph_global_token/pro_graph_global_token: [B, 1, 128]。
        # 该分支建模全局拓扑语义之间的双向依赖，随后送入 CoAttentionLayer
        # 与序列全局特征做二级协同融合。
        ...
        # ── Stream-3: 药物序列 ↔ 蛋白序列 双向交叉注意力 ───────────────────
        # 序列特征为自适应池化后的固定长度表示，仍使用由真实长度映射得到的 mask。
        # if return_attention:
        #     drug_seq_inter, pro_seq_inter, seq_cross_attn = self.seq_bicross(
        #         drug_seq_cnn_feat,
        #         pro_seq_cnn_feat,
        #         a_mask=drug_seq_mask,
        #         b_mask=pro_seq_mask,
        #         return_attn=True
        #     )
        # else:
        #     drug_seq_inter, pro_seq_inter = self.seq_bicross(
        #         drug_seq_cnn_feat,
        #         pro_seq_cnn_feat,
        #         a_mask=drug_seq_mask,
        #         b_mask=pro_seq_mask
        ...
        # seq_inter_out = self.seq_inter_refine(torch.cat([drug_seq_inter, pro_seq_inter], dim=1))
        # pro_graph_inter=pro_graph_inter*pro_graph_global
        # pro_graph_inter=self.gp(pro_graph_inter)
        # drug_graph_inter=drug_graph_inter*drug_graph_global
        # drug_graph_inter=self.gd(drug_graph_inter)
        ...
        # ── MLP 预测 ──────────────────────────────────────────────────────────
        # [B, 256 + 128 + 128 + 128 + 256 + 256] = [B, 1152]
        feat = torch.cat([
            seq_ban_out,   # 序列-序列 BAN        (Stream-1)
            graph_inter_out,  # 图-图双向交互
            # seq_inter_out,    # 序列-序列双向交互
            co_out,           # CKD-style CoAttention 二级融合
            s_gate_out,    # 序列全局互信息         (Stream-4)
            g_gate_out,    # 图全局互信息           (Stream-5)
        ], dim=1)
